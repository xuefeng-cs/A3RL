...
#     def update_actor(self, batch: DatasetDict, importance_sampling_weights) -> Tuple[Agent, Dict[str, float]]:
...
#             return actor_loss, {"actor_loss": actor_loss, "entropy": -(log_probs * importance_sampling_weights).sum()}
...
#     def target_critic_advantage(self, batch: DatasetDict) -> jnp.ndarray:
#         key, rng = jax.random.split(self.rng)
...
#         # print(f'Shape of estimated q(s, a) is {qs.shape}')
#         # * now calculate value
...
#         )
#         num_samples = 20
#         sampled_actions = policy_dist.sample(seed=rng, sample_shape=(num_samples,))
#         sampled_qs = jax.vmap(
...
#         # print(f'shape of sampled_actions is {sampled_actions.shape}, sampled_qs is {sampled_qs.shape}, ensembled: {sampled_qs_ensembled.shape}, sampled_vs: {sampled_vs.shape}')
        
#         # * now calculate advantage
#         return qs - sampled_vs
    
#     def target_critic_td(self, batch: DatasetDict) -> jnp.ndarray:
#         key, rng = jax.random.split(self.rng)
#         target_params = subsample_ensemble(
#             key, self.target_critic.params, self.num_min_qs, self.num_qs
#         )
...
#         print(f'Shape of next_actions: {next_actions.shape}')
#         sampled_qs = jax.vmap(
#             lambda batch_actions: self.target_critic.apply_fn(
#                 {"params": target_params},
...
#         )(next_actions)
#         sampled_qs_ensembled = sampled_qs.min(axis = 1)
#         print(f'Shape of sampled_qs_ensembled: {sampled_qs_ensembled.shape}')
...
#         print(f'Shape of next_q: {next_q.shape}')
        
#         target_q = batch["rewards"] + self.discount * batch["masks"] * next_q
        
#         print(f'Shape of target_q: {target_q.shape}')
...
#             print(f'Shape of new critic_loss: {critic_loss.shape}')
...
#             mini_importance_weights = slice(importance_sampling_weights)
#             new_agent, critic_info = new_agent.update_critic(mini_batch, mini_importance_weights)

#         new_agent, actor_info = new_agent.update_actor(mini_batch, mini_importance_weights)
